...
const NonInjectedCatalogEntityDetails = observer((props: CatalogEntityDetailsProps & Dependencies) => {
  const {
    entity,
    onRun,
    hideDetails,
  ...
  const details = detailItems.get()
    .map((Details, index) => <Details entity={entity} key={index} />);
