...
const actions: ActionTree<ClusterState, any>  = {
  async refreshClusters({ commit }, currentWorkspace: Workspace) {
    const clusters: ClusterInfo[] = await promiseIpc.send('getClusters', currentWorkspace.id).catch(() => false);
    if(!clusters) {
      return false;
    }
  ...
  async removeCluster({getters, dispatch}, id: string) {
    ...
    await promiseIpc.send("removeCluster", cluster.id).catch(() => false);
  ...
  async uploadClusterIcon({commit}, data) {
    ...
    if (res.cluster) {
      commit("updateCluster", res.cluster);
    }
    return res;
  },
  ...
  attachWebview({ commit: _commit }, lens: LensWebview) {
  ...
  destroyWebviews({ commit: _commit }) {
...
const getters: GetterTree<ClusterState, any> = {
  ...
  clusterById: state => (id: string): ClusterInfo => {
    return state.clusters.find(c => c.id === id) || null;
  },
  lenses: state => state.lenses,
  lensById: state => (id: string): LensWebview => {
    return state.lenses.find(c => c.id === id) || null;
  },
};
