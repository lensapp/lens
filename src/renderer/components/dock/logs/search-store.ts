...
const escapingRegex = /[-[\]{}()*+?.,\\^$|#\s]/g;

/**
 * An utility methods escaping user string to safely pass it into new Regex(variable)
 * @param value Unescaped string
 */
export function escapeRegexForSearch(value: string | undefined): RegExp {
  if (value) {
    return new RegExp(value.replace(escapingRegex, "\\$"), "gi");
  }
...
export class LogSearchStore {
  ...
  private findOccurrences(lines: string[], query?: string): number[] {
    const regex = escapeRegexForSearch(query);
