...
export interface DockTabData {
  ...
  /**
   * Addition data is supported
   */
  [field: string]: JsonValue | undefined;
}

export interface DockTabCreateOptions {
  /**
   * Whether to add a number to the end of the tab name if there already exists
   * a tab with that name (ignoring numbers).
   *
   * @default false
   */
  addNumber?: boolean;
...
export class DockStore implements DockStorageState {
  constructor(private readonly dependencies: Dependencies) {
    ...
    // Close tabs with invalid data if validators are provided
  ...
  @computed
  get isOpen(): boolean {
    return this.dependencies.storage.get().isOpen ?? false;
  }
  ...
  @action
  createTab = (rawTabDesc: DockTabCreate, { addNumber = false }: DockTabCreateOptions = {}): DockTabData => {
    const {
      id = uuid.v4(),
      kind,
      pinned = false,
  ...
  closeOtherTabs(tabId: TabId) {
    ...
    this.closeTabs([...this.tabs.slice(0, index), ...this.tabs.slice(index + 1)]);
  }

  closeTabsToTheRight(tabId: TabId) {
    const index = this.getTabIndex(tabId);

    this.closeTabs(this.tabs.slice(index + 1));
  }
   ...
   */
  private persistSelectedTabId(selectedTabId: TabId | undefined): void {
    this.dependencies.storage.merge({ selectedTabId });
  }

  /**
   * Deselects the selected tab and close the dock
   */
  @action
  deselectTab(): void {
    this.persistSelectedTabId(undefined);
    this.close();
  }

  /**
   * Ensures the given tab is selected and the dock is open.
   *
   * If `tabId` is not valid then the state of dock is unchanged.
   * @param tabId The ID of the tab to select
   * @returns true, if `tabId` is valid, else `false`
   */
  @action
  selectTab(tabId: TabId): boolean {
    const id = this.getTabById(tabId)?.id;

    if (id) {
      this.persistSelectedTabId(id);
      this.open();
    }
