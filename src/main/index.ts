...
async function main(): Promise<void> {
  ...
  try {
    ...
    dialog.showErrorBox("Lens Error", `Could not start proxy (127.0.0:${port}): ${error.message || "unknown error"}`);
    app.quit();
  }
  ...
  initMenu({
    logoutHook: async () => {
      ...
      promiseIpc.send('logout', findMainWebContents(), {}).then(() => {
        logger.debug("logout IPC sent");
      });
    },
    showPreferencesHook: async () => {
      // IPC send needs webContents as we're sending it to renderer
      promiseIpc.send('navigate', findMainWebContents(), {name: 'preferences-page'}).then(() => {
        logger.debug("navigate: preferences IPC sent");
      });
    },
    addClusterHook: async () => {
      promiseIpc.send('navigate', findMainWebContents(), { name: "add-cluster-page" }).then(() => {
        logger.debug("navigate: add-cluster-page  IPC sent");
      });
    },
    showWhatsNewHook: async () => {
      promiseIpc.send('navigate', findMainWebContents(), { name: "whats-new-page" }).then(() => {
        logger.debug("navigate: whats-new-page  IPC sent");
      });
    },
    clusterSettingsHook: async () => {
      promiseIpc.send('navigate', findMainWebContents(), { name: "cluster-settings-page" }).then(() => {
