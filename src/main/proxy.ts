...
export class LensProxy {
  ...
  protected buildProxyServer(): http.Server {
    const proxy = this.createProxy();
    const proxyServer = http.createServer(function(req: http.IncomingMessage, res: http.ServerResponse): void {
      this.handleRequest(proxy, req, res);
    }.bind(this));
    proxyServer.on("upgrade", function(req: http.IncomingMessage, socket: Socket, head: Buffer): void {
  ...
  protected createWsListener(): WebSocket.Server {
    ...
    ws.on("connection", ((con: WebSocket, req: http.IncomingMessage): void => {
      const cluster = this.clusterManager.getClusterForRequest(req);
  ...
  protected getNodeParam(requestUrl: string): string {
  ...
  protected async handleRequest(proxy: httpProxy, req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    const contextHandler = cluster.contextHandler;
    try {
      contextHandler.applyAuthHeaders(req);
    } catch (error) {
      res.statusCode = 503;
      res.end();
      return;
  ...
  protected handleWsUpgrade(req: http.IncomingMessage, socket: Socket, head: Buffer): void {
...
export function listen(port: number, clusterManager: ClusterManager): LensProxy {
