...
export class Kubectl {
  ...
  public static bundled(): Kubectl {
    if(!Kubectl.bundledInstance) {
      Kubectl.bundledInstance = new Kubectl(Kubectl.bundledKubectlVersion);
    }
  ...
  public async binDir(): Promise<string | null> {
    try {
      await this.ensureKubectl();
      return this.dirname;
    } catch(err) {
  ...
  public async ensureKubectl(): Promise<boolean> {
    ...
    return lockFile.lock(this.dirname).then(async (release) => {
      ...
      await this.writeInitScripts().catch((error) => {
        logger.error("Failed to write init scripts"); logger.error(error); 
      });
  ...
  protected getRequestOpts(): request.Options {
...
export const bundledKubectl = Kubectl.bundled();
