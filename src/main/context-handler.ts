...
export class ContextHandler {
  ...
  public setClusterPreferences(clusterPreferences?: ClusterPreferences): void {
    this.prometheusProvider = clusterPreferences.prometheusProvider?.type;

    if (clusterPreferences?.prometheus) {
      const { namespace, service, port } = clusterPreferences.prometheus;
      this.prometheusPath = `${namespace}/services/${service}:${port}`;
    } else {
      this.prometheusPath = null;
    }

    this.clusterName = clusterPreferences?.clusterName || this.contextName;
  }
  ...
  public async getPrometheusProvider(): Promise<PrometheusProvider> {
  ...
  public applyAuthHeaders(req: http.IncomingMessage): void {
    req.headers["authorization"] = `Bearer ${this.id}`;
  }

  public async withTemporaryKubeconfig(callback: (kubeconfig: string) => Promise<any>): Promise<void> {
    try {
  ...
  public proxyServerError(): string | null {
    if (!this.proxyServer) {
      return null; 
    }
